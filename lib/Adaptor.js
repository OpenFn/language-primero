...
function execute() {
  ...
  return function (state) {
    return _languageCommon.execute.apply(void 0, [generateAuthString].concat(operations, [cleanupState]))(_objectSpread({}, initialState, {}, state));
  };
}
/**
...
function getCases(query, callback) {
  return function (state) {
    ...
    var expandedQuery = (0, _languageCommon.expandReferences)(query)(state);
    var params = {
      method: 'GET',
    ...
    return new Promise(function (resolve, reject) {
      (0, _request["default"])(requestParams, function (error, response, body) {
        ...
        if (error) {
          ...
          var resp = (0, _Utils.tryJson)(body);
          console.log("Put succeeded: ".concat(response.statusCode, " ").concat(response.statusMessage));
          ...
          if (resp.data.length == 0) {
            ...
            resolve(updateCase(resp.data[0].id, {
              data: function data(state) {
                ...
                var services_section = _data.services_section; // NOTE: When performing an upsert, we only add _new_
                ...
                if (oldServices && oldServices.length > 0) {
                  ...
                  var newServices = services_section.filter(function (os) {
