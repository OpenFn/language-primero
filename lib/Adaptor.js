...
function updateReferral(params, callback) {
  return function (state) {
    ...
    var _expandReferences6 = (0, _languageCommon.expandReferences)(params)(state),
        caseExternalId = _expandReferences6.caseExternalId,
        caseId = _expandReferences6.caseId,
    ...
    if (caseExternalId === 'record_id') {
      ...
      requestParams = {
        method: 'PATCH',
        url: "".concat(url, "/api/v2/cases/").concat(caseId, "/referrals/").concat(id),
        headers: {
          Authorization: auth.token,
          'Content-Type': 'application/json'
        },
        ...
        (0, _request["default"])(requestParams, function (error, response, body) {
          ...
          if (error) {
            ...
            if (resp.data.length == 0) {
              ...
              console.log('Case found. Fetching referrals.');
              var caseRecordId = resp.data[0].id;
              requestParams = {
                method: 'PATCH',
                url: "".concat(url, "/api/v2/cases/").concat(caseRecordId, "/referrals/").concat(id),
                headers: {
                  Authorization: auth.token,
                  'Content-Type': 'application/json'
                },
